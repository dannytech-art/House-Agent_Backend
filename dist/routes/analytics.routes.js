...
class AnalyticsEventModel extends DatabaseService {
    constructor() {
...
class ReportModel extends DatabaseService {
    constructor() {
        super('reports');
    }
    findByType(type) {
...
router.get('/reports', authenticate, requireRole('admin'), async (req, res) => {
    try {
        ...
        let reports;
        if (type) {
            reports = reportModel.findByType(type);
        }
        else {
...
// Generate report
router.post('/reports', authenticate, requireRole('admin'), async (req, res) => {
    try {
        ...
        switch (type) {
            ...
            case 'properties':
                ...
                metrics = {
                    ...
                    byStatus: allProperties.reduce((acc, p) => {
                        acc[p.status] = (acc[p.status] || 0) + 1;
                        return acc;
                    }, {}),
                    byType: allProperties.reduce((acc, p) => {
                        acc[p.type] = (acc[p.type] || 0) + 1;
                        return acc;
                    }, {}),
                };
