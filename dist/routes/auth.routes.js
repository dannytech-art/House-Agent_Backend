...
import { sendOTPEmail, sendWelcomeEmail, sendPasswordResetEmail, generateOTP, isEmailServiceConfigured } from '../services/email.service.js';
const router = Router();
// ============================================
...
router.post('/send-otp', async (req, res) => {
    try {
        ...
        const otp = generateOTP();
        await otpModel.createOTP(email, otp, type, user.id);
        // Send email
...
router.post('/login', async (req, res) => {
    try {
        ...
        if (!user.emailVerified && user.email_verified !== true) {
            // Send new OTP
            const otp = generateOTP();
