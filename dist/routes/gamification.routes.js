...
router.get('/challenges', authenticate, async (req, res) => {
    try {
        ...
        let challenges;
        if (agentId) {
            challenges = challengeModel.findByAgent(agentId);
        }
...
router.post('/challenges', authenticate, requireRole('admin'), async (req, res) => {
    try {
...
router.put('/challenges/:id', authenticate, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        if (updates.progress >= challenge.target && !challenge.completed) {
            ...
            const agent = userModel.findById(challenge.agentId);
            if (agent) {
...
router.get('/quests', authenticate, async (req, res) => {
    try {
        ...
        let quests;
        if (userId) {
            quests = questModel.findByUser(userId);
        }
        else {
...
router.get('/badges', authenticate, async (req, res) => {
    try {
        ...
        const badges = agentId
            ? badgeModel.findByAgent(agentId)
...
// Get territories
router.get('/territories', authenticate, async (req, res) => {
    try {
