...
const validPropertyTypes = ['apartment', 'house', 'duplex', 'penthouse', 'studio', 'land'];
...
router.put('/:id', authenticate, async (req, res) => {
    try {
        ...
        if (updates.minBudget !== undefined)
            updates.minBudget = Number(updates.minBudget);
        if (updates.maxBudget !== undefined)
            updates.maxBudget = Number(updates.maxBudget);
...
router.patch('/:id/fulfill', authenticate, async (req, res) => {
    try {
        const { id } = req.params;
        const request = propertyRequestModel.findById(id);
        if (!request) {
...
//# sourceMappingURL=property-request.routes.js.map