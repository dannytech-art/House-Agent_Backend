...
import { uploadToCloudinary, uploadMultipleToCloudinary, deleteFromCloudinary, getCloudinaryStatus, } from '../services/upload.service.js';
const router = Router();
...
// Upload single file
router.post('/single', authenticate, upload.single('file'), handleUploadError, async (req, res) => {
    try {
        if (!req.file) {
...
router.post('/multiple', authenticate, upload.array('files', 10), handleUploadError, async (req, res) => {
    try {
...
router.post('/property-media', authenticate, upload.array('files', 10), handleUploadError, async (req, res) => {
    try {
        const files = req.files;
        if (!files || files.length === 0) {
            return res.status(400).json({
