...
router.get('/kyc/review', authenticate, requireRole('admin'), async (req: AuthRequest, res: Response) => {
  try {
    const { status } = req.query;

    let kycs;
    if (status) {
      kycs = kycModel.findByStatus(status as string);
    } else {
...
router.post('/kyc/review', authenticate, requireRole('admin'), async (req: AuthRequest, res: Response) => {
  try {
...
router.post('/kyc/submit', authenticate, requireRole('agent'), async (req: AuthRequest, res: Response) => {
  try {
...
// Get flags
router.get('/flags', authenticate, requireRole('admin'), async (req: AuthRequest, res: Response) => {
  try {
...
router.post('/flags', authenticate, async (req: AuthRequest, res: Response) => {
  try {
...
// Update flag (review)
router.put('/flags/:id', authenticate, requireRole('admin'), async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
...
router.get('/settings', authenticate, requireRole('admin'), async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (key) {
      const setting = settingModel.findByKey(key as string);
      res.json({
        success: true,
        data: setting,
      });
