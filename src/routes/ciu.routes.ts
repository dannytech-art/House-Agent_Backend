...
// CIU Models
interface ClosableDealDocument {
  id: string;
  propertyId: string;
  agentId: string;
  urgency: 'low' | 'medium' | 'high' | 'critical';
  closingProbability: number;
  status: 'active' | 'in-progress' | 'closed' | 'lost';
  assignedTo?: string;
  notes?: string;
  createdAt: string;
  updatedAt: string;
...
interface RiskFlagDocument {
  id: string;
  entityType: string;
  entityId: string;
  type: 'fraud' | 'duplicate' | 'suspicious' | 'compliance' | 'other';
...
class ClosableDealModel extends DatabaseService<ClosableDealDocument> {
  constructor() {
    super('closable_deals');
  }

  findByAgent(agentId: string): ClosableDealDocument[] {
    return this.findMany(d => d.agentId === agentId);
  }

  findActive(): ClosableDealDocument[] {
    return this.findMany(d => d.status === 'active' || d.status === 'in-progress');
  }

  findByUrgency(urgency: string): ClosableDealDocument[] {
    return this.findMany(d => d.urgency === urgency);
  }
}

class VilanowTaskModel extends DatabaseService<VilanowTaskDocument> {
  constructor() {
    super('vilanow_tasks');
  }

  findByAgent(agentId: string): VilanowTaskDocument[] {
    return this.findMany(t => t.assignedAgent === agentId);
  }

  findPending(): VilanowTaskDocument[] {
    return this.findMany(t => t.status === 'pending' || t.status === 'in-progress');
  }
}

class RiskFlagModel extends DatabaseService<RiskFlagDocument> {
  constructor() {
    super('risk_flags');
  }

  findActive(): RiskFlagDocument[] {
    return this.findMany(r => r.status === 'active' || r.status === 'investigating');
  }

  findByEntity(entityType: string, entityId: string): RiskFlagDocument[] {
    return this.findMany(r => r.entityType === entityType && r.entityId === entityId);
  }
}

class AutomationRuleModel extends DatabaseService<AutomationRuleDocument> {
  constructor() {
    super('automation_rules');
  }

  findEnabled(): AutomationRuleDocument[] {
...
// Get closable deals
router.get('/deals', authenticate, requireRole('admin', 'agent'), async (req: AuthRequest, res: Response) => {
  try {
...
router.post('/deals', authenticate, requireRole('admin'), async (req: AuthRequest, res: Response) => {
  try {
...
router.put('/deals/:id', authenticate, requireRole('admin'), async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
...
// Get tasks
router.get('/tasks', authenticate, requireRole('admin', 'agent'), async (req: AuthRequest, res: Response) => {
  try {
...
// Create task
router.post('/tasks', authenticate, requireRole('admin'), async (req: AuthRequest, res: Response) => {
  try {
...
// Get risk flags
router.get('/risks', authenticate, requireRole('admin'), async (req: AuthRequest, res: Response) => {
  try {
...
// Create risk flag
router.post('/risks', authenticate, requireRole('admin'), async (req: AuthRequest, res: Response) => {
  try {
...
// Update risk flag
router.put('/risks/:id', authenticate, requireRole('admin'), async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
    const updates = req.body;
...
router.get('/automation', authenticate, requireRole('admin'), async (req: AuthRequest, res: Response) => {
  try {
...
router.post('/automation', authenticate, requireRole('admin'), async (req: AuthRequest, res: Response) => {
  try {
...
router.put('/automation/:id', authenticate, requireRole('admin'), async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
    const updates = req.body;
...
router.delete('/automation/:id', authenticate, requireRole('admin'), async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
