...
router.get('/offers', authenticate, async (req: AuthRequest, res: Response) => {
  try {
...
router.post('/offers', authenticate, requireRole('agent'), async (req: AuthRequest, res: Response) => {
  try {
...
// Purchase offer
router.post('/offers/:offerId/purchase', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const buyer = userModel.findById(req.userId!) as any;
    ...
    const seller = userModel.findById(offer.agentId) as any;
    if (seller) {
...
// Get collaborations
router.get('/collaborations', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const { agentId, type, status } = req.query;

    let collaborations = agentId
      ? collaborationModel.findByAgent(agentId as string)
...
router.post('/collaborations', authenticate, requireRole('agent'), async (req: AuthRequest, res: Response) => {
  try {
...
router.put('/collaborations/:id', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
