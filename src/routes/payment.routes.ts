...
router.post('/verify-inline', async (req: Request, res: Response) => {
  try {
    ...
    if (transaction.status === 'completed') {
      const agent = await userModel.findById(transaction.userId) as any;
      return res.json({
        success: true,
  ...
  try {
    ...
    if (!transaction) {
      ...
      transaction = await transactionModel.findOne(tx => {
        const meta = tx.metadata || {};
        return (
          (meta.transactionId === paymentData.metadata?.transactionId || tx.id === paymentData.metadata?.transactionId) &&
          tx.userId === req.userId
        );
      });
  ...
  try {
    ...
    if (event.event === 'charge.success') {
      ...
      if (!transaction) {
        ...
        const agent = await userModel.findById(userId) as any;
        if (agent) {
