...
// Location model for territory locations
...
class LocationModel extends DatabaseService<LocationDocument> {
  constructor() {
    super('locations');
  }

  findByArea(area: string): LocationDocument[] {
    return this.findMany(loc => 
      loc.area.toLowerCase().includes(area.toLowerCase())
    );
  }

  findByState(state: string): LocationDocument[] {
...
// Claim territory
router.post('/claim', authenticate, requireRole('agent'), async (req: AuthRequest, res: Response) => {
  try {
    ...
    const existingTerritory = territoryModel.findOne(
      t => t.agentId === req.userId && t.area.toLowerCase() === area.toLowerCase()
    );
...
router.get('/my', authenticate, requireRole('agent'), async (req: AuthRequest, res: Response) => {
  try {
...
router.get('/locations', async (req, res: Response) => {
  try {
    ...
    let locations;
    if (area) {
      locations = locationModel.findByArea(area as string);
    } else if (state) {
      locations = locationModel.findByState(state as string);
    } else if (search) {
      locations = locationModel.findMany(loc => 
        loc.name.toLowerCase().includes((search as string).toLowerCase()) ||
        loc.area.toLowerCase().includes((search as string).toLowerCase())
      );
    } else {
...
// Get location by ID
router.get('/locations/:id', async (req, res: Response) => {
  try {
    const { id } = req.params;
